					...
					local function SafeGetMsg()
						...
						if type(v) == "table" then
							if v.IsPlayer and v:IsPlayer() then
								local CorrectClient = nil
								if v:Getyy() then
									CorrectClient = v:Getyy():GetClient()
								end
...
local function on_coroutine_error( Error )
	local Error, btinfo = debug.excepthook( Error )
	LOG.serverError(btinfo)
end

function coroutine.create( func )
	local _func = function(...)
		local ret = {xpcall(function() return func(unpack(arg)) end, on_coroutine_error),}
		if ret[1] then
			table.remove (ret, 1)
			return unpack(ret)
		end
...
function SafeY1Except(Error)
	_RUNTIME_ERROR("ERROR!!", Error )
	local btinfo = nil
	local function Xy3Except() 
		--内服才需要进入调试器。
		if IsInternalServer and IsInternalServer() and  engine.GetDebugLevel() < _DDEBUG then
			OnExcept(Error, true)
		else
			btinfo = OnExcept(Error, false, 3)
	...
	xpcall(Xy3Except, function(Error) _RUNTIME_ERROR("intrnal excepthook error", Error) end)
...
debug.verify = function(cond, msg)
	if cond then
		return true
	end

	xpcall(function()
		error('verify fail:' .. msg or '')
...
function debug.verbose_except_hook(Msg)
	...
	while 1 do
		...
		if finfo.what == 'Lua' then
			...
			while 1 do
				...
				local vardesc = string.format('\t\t %s => %s', k, tostring(v))
				M(vardesc)
				rid = rid + 1
			end
		end
	...
	_RUNTIME_ERROR(table.concat(T, '\n'))
	return Msg
end

local function get_object_friend_string(obj)
	if type(obj) == "table" then
		if obj.IsPlayer and obj:IsPlayer() then
			local CorrectClient = nil
			if obj:Getyy() then
				CorrectClient = obj:Getyy():GetClient()
			end
			return string.format("userid=%d,name=%s,URS=%s,CorrectClient=%s", obj:GetId() or 0, 
					obj:GetName() or "unknow", obj:GetURS() or "unknow", tostring(CorrectClient) )
		elseif obj.IsClass then
			local Id = "none"
			if obj.GetId then
				Id = obj:GetId()
			end
			return string.format("objectid=%s", tostring(Id))
		end
	end
	return nil
end
	...
	while 1 do
		...
		if finfo.what == 'Lua' then
			...
			while 1 do
				local k, v = debug.getlocal(l, rid)
				if not k then break end
				local desc = get_object_friend_string(v) or tostring(v)
...
--这个函数不能返回nil，只能返回true或者false
--返回true则忽略这个报警
function debug.is_ignore_exceedtime_log(err_str)
	local comps = string.split(err_str, '\n')
	if comps[5] == "\t[2] [C]:-1 --> loadfile" then
		return true
	end
	return false
end

debug.excepthook = SafeY1Except
