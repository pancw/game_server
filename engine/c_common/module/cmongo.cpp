...
bool connectMongo(const char* url)
{
	mongoc_uri_t *uri;
	bson_error_t error;
...
static mongoc_collection_t* tryInitCollection(const char* col)
{
	if (!client)
	{
		return NULL;
...
static int insertCol(lua_State *L) 
{
...
static int findCol(lua_State *L) 
{
	const char* colName = lua_tostring(L, 1);
	if (!colName)
	{
...
void luaopenMongo(lua_State* L)
{
	luaL_register(L, "CMongo", lualib);	
}
